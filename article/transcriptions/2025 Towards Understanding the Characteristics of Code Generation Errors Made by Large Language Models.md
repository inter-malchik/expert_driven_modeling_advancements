Ниже представлен структурированный документ Markdown, созданный на основе предоставленной статьи, с точным соблюдением всех ваших инструкций.
...
zhijie.wang@ualberta.ca, zijiez4@illinois.edu, dsong4@ualberta.ca, yuhenghuang42@g.ecc.u-tokyo.ac.jp, chen3301@purdue.edu, ma.lei@acm.org, tianyi@purdue.edu
...
*   We established a taxonomy of both syntactic and semantic characteristics of code generation errors through open coding and thematic analysis. Our labeling results are available at a GitHub repository [8].
...
---
*Сноска:*
† The first three authors contributed equally to this work. Zijie Zhou was a remote research intern at Purdue University.
---
...
In this study, we focus on six representative code generation LLMs: CodeGen-16B [1], InCoder-1.3B [2], GPT-3.5 [5], GPT-4 [4], SantaCoder [6], and StarCoder [3]. As shown in Table I, these models cover a wide range of model sizes and model performance. CodeGen-16B was trained on $217$GB Python code from BigPython [1]. InCoder-1.3B was trained on $159$GB of open-source repositories from GitHub, GitLab, and StackOverflow. SantaCoder and StarCoder were trained on The Stack dataset [11]. The training data of GPT-3.5 and GPT-4 are currently unknown. As GPT-3.5 and GPT-4 are constantly evolving, we used GPT-3.5-Turbo-0301 and GPT-4-0314, the two most recent model checkpoints at the time of our analysis.

**TABLE I: Code generation LLMs used in this study**

| Model | Release | Size | Performance (Pass@1) | Performance (Incorrect Solutions) |
| :--- | :--- | :--- | :--- | :--- |
...
**Iterative refinement of the codebook.** After obtaining the initial codebook, we invited another two authors to iteratively improve the codebook. The four authors first independently analyzed $10$ incorrect code snippets following the same procedure described above and labeled the error characteristics based on the initial codebook. If a new characteristic was identified, an author created a new label to describe the characteristic.
...
Note that for some tasks, an LLM may propose an alternative solution with errors compared with the ground-truth solution in HumanEval. We identified $17$ incorrect solutions where the LLM proposed an alternative way to solve the task but did not correctly solve it. In such cases, it is unfair to directly compare the incorrect code with the ground truth. To address this issue, one author manually solved the task following the LLM’s solution and computed the metrics by comparing the incorrect solution with the alternative, correct solution.
...
*   **Reference Error** involves incorrect references to variables or functions, which includes the usage of an incorrect function or variable that does not match the requirement (*wrong method/variable*) and reference to a variable or method name that has not been defined (*undefined name*).
...
| :--- | :--- | :--- | :--- |
| Condition Error | Missing condition | `# [Task 151] CodeGen-16b`<br>`def double_the_difference(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`sum = 0`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for i in lst:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if i > 0 and i % 2 != 0:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`sum += i ** 2`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return sum` | `# [Task 151] Ground Truth`<br>`def double_the_difference(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`ans = 0`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for num in lst:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if num % 2 == 1 and num > 0 and "." not in str(num): ans += num ** 2`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return ans` |
| Condition Error | Incorrect condition | `# [Task 44] GPT-3.5`<br>`def change_base(x, base):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`r = ""`
&nbsp;&nbsp;&nbsp;&nbsp;`while x > 0:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`rem, r = x % base, str(rem)+r`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`x = x // base`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return result` | `# [Task 44] Ground Truth`<br>`def change_base(x, base):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if x == 0: return "0"`<br>&nbsp;&nbsp;&nbsp;&nbsp;`ret = ""`
&nbsp;&nbsp;&nbsp;&nbsp;`while x != 0:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`ret, x = str(x%base) + ret, x // base`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return ret` |
| Constant Value Error | Constant value error | `# [Task 51] SantaCoder`<br>`def remove_vowels(text):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`vowels = 'aeiou'`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return ''.join([char for char in text if char not in vowels])` | `# [Task 51] Ground Truth`<br>`def remove_vowels(text):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return "".join(list(filter(lambda ch: ch not in "aeiouAEIOU", text)))` |
| Reference Error | Wrong method/variable | `# [Task 79] InCoder-1B`<br>`def decimal_to_binary(decimal):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return str(decimal) + "db"` | `# [Task 79] Ground Truth`<br>`def decimal_to_binary(decimal):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return bin(decimal) + "db"` |
| Reference Error | Undefined name | `# [Task 83] CodeGen-16B`<br>`def starts_one_ends(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if n == 1: return 1`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return starts_one_ends(n-1)+ ends_one_starts(n-1)` | `# [Task 83] Ground Truth`<br>`def starts_one_ends(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if n == 1: return 1`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return 18 * 10 ** (n - 2)` |
| Operation/<br>Calculation Error | Incorrect arithmetic operation | `# [Task 44] InCoder-1B`<br>`def change_base(x: int, base: int):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`digits = []`<br>&nbsp;&nbsp;&nbsp;&nbsp;`while x:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`digits.append(str(x % base))`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`x /= base`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return ''.join(reversed(digits))` | `# [Task 44] Ground Truth`<br>`def change_base(x: int, base: int):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if x == 0: return "0"`<br>&nbsp;&nbsp;&nbsp;&nbsp;`ret = ""`
&nbsp;&nbsp;&nbsp;&nbsp;`while x != 0:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`ret = str(x % base) + ret`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`x //= base`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return ret` |
| Operation/<br>Calculation Error | Incorrect comparison operation | `# [Task 138] CodeGen-16B`<br>`def is_equal_to_sum_even(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return n <= 8 and n % 2 == 0` | `# [Task 138] Ground Truth`<br>`def is_equal_to_sum_even(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return n >= 8 and n % 2 == 0` |
| Garbage Code | Only comments | `# [Task 152] InCoder-1B`<br>`def compare(game,guess):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`# Regenerate the task description.` | `# [Task 152] Ground Truth`<br>`def compare(game,guess):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return [abs(game[i] - guess[i]) for i in range(len(game))]` |
| Garbage Code | Meaningless code snippet | `# [Task 138] StarCoder`<br>`def is_equal_to_sum_even(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`pass` | `# [Task 138] Ground Truth`<br>`def is_equal_to_sum_even(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return n >= 8 and n % 2 == 0` |
| Garbage Code | Wrong (logical) direction | `# [Task 20] InCoder-1.3B`<br>`def find_closest_elements(numbers):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`closest_to_one, closest_to_two = numbers[0], numbers[1]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for number in numbers:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if number < closest_to_one:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`closest_to_one = number`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if number > closest_to_two:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`cloeset_to_two = number`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return closest_to_one, closest_to_two` | `# [Task 20] InCoder-1.3B`<br>`def find_closest_elements(numbers):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`min_diff, min_pair = float("inf"), None`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for l, r in zip(numbers[:-1], numbers[1:]):`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`diff = r - l`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if diff < min_diff:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`min_diff = diff`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`min_pair = (l, r)`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return min_pair` |
| Incomplete Code/<br>Missing Steps | Missing one step | `# [Task 16] InCoder-1B`<br>`def count_distinct_chars(string):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return len(set(string))` | `# [Task 16] Ground Truth`<br>`def count_distinct_chars(string):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return len(set(string.lower()))` |
| Incomplete Code/<br>Missing Steps | Missing multiple steps | *(Combined with the row above in original table formatting)* | *(Combined with the row above in original table formatting)* |
| Memory Error | Infinite loop | `# [Task 100] CodeGen-16b`<br>`def make_a_pile(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if n % 2 == 0:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`return [n] + make_a_pile(n+2)`<br>&nbsp;&nbsp;&nbsp;&nbsp;`else:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`return [n] + make_a_pile(n+1)` | `# [Task 100] Ground Truth`<br>`def make_a_pile(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`ans, num = [], n`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for _ in range(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`ans.append(num)`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`num += 2`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return ans` |
...
| Error Characteristic | | Example of Incorrect Solutions | Correct Solution |
| :--- | :--- | :--- | :--- |
| Conditional Error | If error | `# [Task 151] CodeGen-16b`<br>`def double_the_difference(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`sum = 0`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for i in lst:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if i > 0 and i % 2 != 0:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`sum += i ** 2`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return sum` | `# [Task 151] Ground Truth`<br>`def double_the_difference(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`ans = 0`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for num in lst:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if num%2==1 and num>0 and "." not in str(num): ans += num ** 2`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return ans` |
| Loop Error | For error | `# [Task 121] GPT-3.5`<br>`def solution(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`sum = 0`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for i in range(1, len(lst), 2):`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if lst[i] % 2 != 0: sum +=lst[i]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return sum` | `# [Task 121] Ground Truth`<br>`def solution(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return sum([x for idx, x in enumerate(lst) if idx%2==0 and x%2==1])` |
| Loop Error | While error | *(Combined with the row above in original table formatting)* | *(Combined with the row above in original table formatting)* |
| Return Error | Incorrect return value | `# [Task 103] GPT-3.5`<br>`def rounded_avg(n, m)`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if n > m: return -1`<br>&nbsp;&nbsp;&nbsp;&nbsp;`avg=round(sum(range(n,m+1))/(m-n+1))`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return bin(avg)[2:]` | `# [Task 103] Ground Truth`<br>`def rounded_avg(n, m)`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if n > m: return -1`<br>&nbsp;&nbsp;&nbsp;&nbsp;`avg = round((n + m) / 2)`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return bin(avg)` |
| Method Call Error | Incorrect function name | `# [Task 54] StarCoder`<br>`def same_chars(s0, s1):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return sorted(s0) == sorted(s1)` | `# [Task 54] Ground Truth`<br>`def same_chars(s0, s1):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return set(s0) == set(s1)` |
| Method Call Error | Incorrect function arguments | *(Combined with the row above in original table formatting)* | *(Combined with the row above in original table formatting)* |
| Method Call Error | Incorrect method call target | *(Combined with the row above in original table formatting)* | *(Combined with the row above in original table formatting)* |
| Assignment Error | Incorrect constant | `# [Task 138] InCoder-1.3B`<br>`def is_equal_to_sum_even(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return n >= 4 and n % 2 == 0` | `# [Task 138] Ground Truth`<br>`def is_equal_to_sum_even(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return n >= 8 and n % 2 == 0` |
| Assignment Error | Incorrect arithmetic | *(Combined with the row above in original table formatting)* | *(Combined with the row above in original table formatting)* |
| Assignment Error | Incorrect variable name | *(Combined with the row above in original table formatting)* | *(Combined with the row above in original table formatting)* |
| Assignment Error | Incorrect comparison | *(Combined with the row above in original table formatting)* | *(Combined with the row above in original table formatting)* |
| Import Error | Import error | `# [Task 133] StarCoder`<br>`def sum_squares(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return sum[int(math.ceil(i)**2 for i in lst]` | `# [Task 133] Ground Truth`<br>`def sum_squares(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`import math`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return sum(map(lambda x: math.ceil(x)**2, lst))` |
| Code Block Error | Incorrect code block | `# [Task 83] InCoder-1.3B`<br>`def starts_one_ends(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`count = 0`<br>&nbsp;&nbsp;&nbsp;&nbsp;`while n > 0:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`count, n = count + 1, n / 10`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return count` | `# [Task 83] Ground Truth`<br>`def starts_one_ends(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if n == 1:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`return 1`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return 18 * 10 ** (n - 2)` |
| Code Block Error | Missing code block | `# [Task 60] CodeGen-16B`<br>`def next_smallest(lst):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if len(lst)<2: return None`<br>&nbsp;&nbsp;&nbsp;&nbsp;`lst.sort()`<br>&nbsp;&nbsp;&nbsp;&nbsp;`return lst[1]` | `# [Task 60] Ground Truth`<br>`def is_prime(n):`<br>&nbsp;&nbsp;&nbsp;&nbsp;`if len(lst)<=1: return None`<br>&nbsp;&nbsp;&nbsp;&nbsp;`sorted_list=sorted(lst)`<br>&nbsp;&nbsp;&nbsp;&nbsp;`for x in sorted_list:`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`if x!=sorted_list[0]: return x` |
...
> **Рисунок 1: Distribution of semantic characteristics of code generation errors made by six LLMs.**
> *Описание изображения:* Рисунок состоит из шести горизонтальных столбчатых диаграмм (a-f), отображающих распределение семантических характеристик ошибок генерации кода для моделей: CodeGen-16B, InCoder-1.3B, GPT-3.5, GPT-4, SantaCoder, StarCoder. Ось Y на каждом графике перечисляет типы ошибок (например, "Wrong (logical) direction", "Missing multiple steps", "Incorrect condition", "Meaningless code snippet" и др.), а ось X показывает процентное соотношение. Для всех моделей топовыми ошибками являются логические: "Wrong (logical) direction" и "Incorrect condition" (занимая от 15% до 35% доли ошибок). Модели меньшего размера (CodeGen-16B, InCoder-1.3B, SantaCoder, StarCoder) имеют значительную долю ошибок "Missing multiple steps" (около 10-17%) и "Meaningless code snippet" (до 14%). Для GPT-4 ошибка "Wrong (logical) direction" занимает 35.0%, а "Incorrect condition" 15.0%, при этом ошибки типа "Meaningless code snippet" полностью отсутствуют.
...
> **Рисунок 2: Distribution of syntactic characteristics of code generation errors made by six LLMs.**
> *Описание изображения:* Рисунок состоит из шести горизонтальных столбчатых диаграмм (a-f), отображающих распределение синтаксических характеристик ошибок (местоположения ошибок) для тех же шести моделей. Ось Y перечисляет типы ошибок (например, "Missing code block", "Incorrect code block", "If error", "Incorrect function name", "Incorrect function arguments"), а ось X показывает проценты. Абсолютным лидером среди всех моделей являются "Missing code block" и "Incorrect code block" (в сумме более 40% ошибок для каждой модели). На втором и третьем местах часто встречаются "If error" (от 10% до 20%) и проблемы с аргументами функций или названиями функций. GPT-4 показывает большую концентрацию ошибок в нескольких категориях (Incorrect code block - 35.0%, Missing code block - 25.0%, If error - 20.0%), в то время как ошибки малых моделей распределены по большему числу категорий, включая "While error" и "Import error".
...
> **Рисунок 3: Mappings between semantic and syntactic error characteristics of code generation errors made by six LLMs.**
> *Описание изображения:* Это серия из шести диаграмм Сэнкэй (a-f) для шести исследуемых моделей. Слева на каждой диаграмме расположены узлы "Семантические характеристики" (Constant error, Incorrect condition, Missing condition, Wrong direction, Meaningless code и др.). Справа — узлы "Синтаксические характеристики" (For error, If error, Incorrect code block, Missing code block, Incorrect args и др.). Цветные ленты соединяют левые и правые узлы, показывая, как семантическая причина проявляется в коде. Отчетливо видно толстые связи между "Wrong direction" (семантика) и "Incorrect code block" (синтаксис), а также между "Missing multiple steps" и "Missing code block". Также показано, что одна синтаксическая ошибка, такая как "Incorrect args" или "If error", может исходить из множества разных семантических узлов (например, от "Constant error", "Incorrect condition" и "Incorrect arithmetic").
...
> **Рисунок 4: Levenshtein distance between the incorrect code and correct code. The vertical dashed lines indicate the medians.**
> *Описание изображения:* Шесть гистограмм (a-f), отображающих распределение расстояния Левенштейна (по оси X, значения от 0 до 400) между ошибочно сгенерированным кодом и правильным ответом для каждой LLM. Ось Y показывает процентное распределение. На каждой гистограмме отмечена медиана вертикальной пунктирной линией. Для большинства моделей медиана находится в районе 100-150 правок, а распределение имеет длинный "хвост" вправо вплоть до 300-400 правок. У моделей GPT-3.5 и GPT-4 медианы смещены немного правее (около 130-150) по сравнению с малыми моделями (например, InCoder-1.3B или SantaCoder, где медианы ближе к 100). Это означает, что когда большие модели ошибаются, их отклонение в тексте от эталона может быть больше.

As shown in Fig. 5, the median CodeBERTScore values are approximately $0.2$ for the six LLMs, except for GPT-3.5, which has a median similarity of $0.05$. Additionally, a large portion of CodeBERTScore values across all six LLMs falls below $0.1$. According to [22], a CodeBERTScore value of $0$ indicates that two code snippets are unrelated, while a value of $1$ indicates that the snippets are exactly the same. These results indicate that the majority of the incorrect solutions deviate significantly in semantics from the correct solutions. Overall, both low textual and semantic similarities suggest that the incorrect code generated by LLMs often exhibits big differences from the correct solutions, not just minor errors. Such observations are also aligned with our findings in RQ1—LLMs tend to make non-trivial errors such as *missing multiple steps* and *wrong logical direction*. Addressing these issues would require substantial edits. For instance, the median number of edits to fix errors of *missing multiple steps* is $108$ edits.

> **Рисунок 5: CodeBERTScore between the incorrect code and correct code. The vertical dashed lines indicate the medians.**
> *Описание изображения:* Шесть гистограмм (a-f), отображающих метрику семантического сходства CodeBERTScore (ось X от 0.0 до 1.0) для ошибок моделей. Ось Y — это процентное соотношение. Большинство распределений сильно скошены влево, сосредотачиваясь в районе значений 0.0 – 0.2. Вертикальные пунктирные линии показывают медиану, которая почти для всех моделей (кроме GPT-3.5) находится около значения 0.2, а у GPT-3.5 медиана около 0.05. Очень мало ошибок имеют показатель CodeBERTScore выше 0.6. Это подтверждает, что ошибочный код семантически далек от эталонного решения.
...
Following the automated program repair literature [23], we also classify all incorrect code snippets into three categories based on the effort required to fix them: (1) single-line errors, (2) single-hunk errors, and (3) multi-hunk errors. Specifically, a "hunk" refers to several contiguous lines in a program. Fig. 6 shows the distribution. Overall, the majority of errors are single-hunk or multi-hunk errors, which require substantial effort to repair compared with single-line errors. Compared with other LLMs, GPT-3.5 and GPT-4 exhibit a more balanced distribution among the three categories. SantaCoder made the most single-line errors ($35\%$). StarCoder made the most single-hunk errors ($57\%$). Notably, the least accurate model, InCoder-1.3B, made the most multi-hunk errors ($41\%$).

> **Рисунок 6: Distribution of single-line, single-hunk, and multi-hunk faults in different LLMs**
> *Описание изображения:* Горизонтальная 100%-ная составная столбчатая диаграмма для шести моделей. Столбцы разделены на три цветовые секции: single-line (одна строка), single-hunk (один блок), multi-hunk (несколько блоков).
> Данные по моделям:
> StarCoder: 19% (single-line), 57% (single-hunk), 24% (multi-hunk).
> SantaCoder: 35%, 53%, 13%.
> GPT-4: 26%, 37%, 37%.
> GPT-3.5: 33%, 33%, 33%.
> InCoder-1.3B: 17%, 42%, 41%.
> CodeGen-16B: 25%, 48%, 27%.
...
We investigated whether there was a significant difference between the complexity of successfully solved tasks and the complexity of failed tasks. We ran the Mann-Whitney *U*-test to examine the statistical difference in task complexity for each LLM. Fig. 7 shows the results. We observed a statistically significant difference in both prompt length and LOC of correct solutions among all six LLMs. Specifically, the p-values for prompt length are $4\text{e-}9$, $5\text{e-}9$, $2\text{e-}5$, $2\text{e-}10$, $2\text{e-}9$, and $5\text{e-}4$; and the effect sizes are $0.82$, $1.38$, $0.71$, $0.77$, $1.32$, and $0.75$ for the six models in the order listed in Fig. 7a. The p-values for LOC of correct solutions are $3\text{e-}6$, $4\text{e-}5$, $6\text{e-}3$, $1\text{e-}2$, $1\text{e-}5$, and $1\text{e-}4$; and the effect sizes are $0.72$, $0.77$, $0.63$, $0.65$, $0.69$, and $0.73$, respectively. The number of AST nodes shows a similar distribution as LOC (Fig. 7c), where the correct solutions of successfully solved tasks have significantly more AST nodes. The p-values are $2\text{e-}7$, $4\text{e-}5$, $6\text{e-}3$, $1\text{e-}2$, $1\text{e-}5$, and $1\text{e-}4$; the effect sizes are $0.65$, $0.88$, $0.30$, $0.43$, $0.64$, and $0.74$. We further calculated the Pearson correlation coefficient between the number of AST nodes and LOC, which shows a statistically significant correlation ($r=0.864$, $p=4\text{e-}50$). This shows the consistency of our findings based on different metrics.

> **Рисунок 7: Distribution of passed/failed tasks’ complexity**
> *Описание изображения:* Рисунок состоит из трех графиков типа "скрипичная диаграмма" (Violin plots).
> (a) Prompt length: Ось Y показывает длину промпта в токенах (от 0 до 200). Скрипичные графики разделены на Pass (синий) и Fail (оранжевый) для каждой из 6 моделей. Распределения Fail-задач явно смещены вверх, имея бОльшую медиану и расширения в верхних значениях, что указывает на то, что задачи с длинными промптами чаще проваливаются.
> (b) LOC of correct solutions: Ось Y показывает количество строк кода (от 0 до 16+). Аналогично, для всех моделей распределение Fail смещено к более высоким значениям LOC по сравнению с Pass.
> (c) Number of AST nodes of correct solutions: Ось Y показывает количество узлов абстрактного синтаксического дерева (от 0 до 200+). Графики Fail также показывают более высокие средние значения по сравнению с Pass, подтверждая результаты LOC.
...
> **Рисунок 8: Semantic characteristics vs. test pass rates**
> *Описание изображения:* Горизонтальная 100%-ная составная столбчатая диаграмма. Ось Y представляет различные семантические ошибки. Ось X показывает процентное соотношение. Цветовые сегменты: Completely Failed (синий), [75%, 100%) Failed (светло-оранжевый), [50%, 75%) Failed (зеленый), [25%, 50%) Failed (красный) и (0, 25%) Failed (фиолетовый). Ошибки "Only comments" и "Meaningless code" ведут к 100% или 84% Complete Failure. "Undefined name" имеет 76% полного провала. Напротив, "Missing condition", "Missing mult steps", "Incorrect condition" показывают высокий процент частичного прохождения тестов (менее 30% Complete Failure).
...
> **Рисунок 9: Syntactic characteristics vs. test pass rates**
> *Описание изображения:* Горизонтальная 100%-ная составная столбчатая диаграмма. Ось Y представляет различные синтаксические ошибки. Ось X показывает проценты. Цветовая шкала как на Рисунке 8 (от Completely Failed до малого процента падений). Ошибки "Import error" (62%), "Incorrect func name" (55%), "Incorrect arith" (53%) имеют наибольший процент Completely Failed. Напротив, "If error" имеет самый низкий показатель Completely Failed — всего 18%, что означает, что 82% таких ошибок приводят лишь к частичному провалу тестов.
...
This work was supported in part by Canada CIFAR AI Chairs Program, Natural Sciences and Engineering Research Council of Canada, JST CRONOS Grant (No.JPMJCS24K8), JSPS KAKENHI Grant (No.JP21H04877, No.JP23H03372, and No.JP24K02920), and the Autoware Foundation. This work was also supported in part by NSF CCF-2340408.
...
[20] R. Abreu, P. Zoeteweij, and A. J. Van Gemund, “On the accuracy of spectrum-based fault localization,” in *Testing: Academic and industrial conference practice and research techniques-MUTATION (TAICPART-MUTATION 2007)*. IEEE, 2007, pp. 89–98.
...
[46] Y. Ding, B. Steenhoek, K. Pei, G. Kaiser, W. Le, and B. Ray, “Traced: Execution-aware pre-training for source code,” in *Proceedings of the 46th IEEE/ACM International Conference on Software Engineering*, 2024, pp. 1–12.
...
[50] B. Roziere et al., “Unsupervised translation of programming languages,” *Advances in neural information processing systems*, vol. 33, pp. 20 601–20 611, 2020.
[51] A. Lozhkov, R. Li et al., “Starcoder 2 and the stack v2: The next generation,” *arXiv preprint arXiv:2402.19173*, 2024.
...
[66] S. Arakelyan, R. Das, Y. Mao, and X. Ren, “Exploring distributional shifts in large language models for code analysis,” in *Proceedings of the 2023 Conference on Empirical Methods in Natural Language Processing*, 2023, pp. 16 298–16 314.
...
[69] B. Steenhoek, M. M. Rahman et al., “A comprehensive study of the capabilities of large language models for vulnerability detection,” *arXiv preprint arXiv:2403.17218*, 2024.
...
[75] R. Pan, A. R. Ibrahimzada et al., “Lost in translation: A study of bugs introduced by large language models while translating code,” in *Proceedings of the IEEE/ACM 46th International Conference on Software Engineering*, 2024, pp. 1–13.
...
[92] J. Wei, X. Wang, D. Schuurmans et al., “Chain-of-thought prompting elicits reasoning in large language models,” *Advances in Neural Information Processing Systems*, vol. 35, pp. 24 824–24 837, 2022.