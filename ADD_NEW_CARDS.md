...
`article/commentary_colors.py` перебирает `COMMENTARY_IDS = [f"n{i}" for i in range(1, 49)]` и раскладывает оттенки по кругу. Сейчас карточек 48, диапазон `1..49` (не включая 49) заполнен полностью.

Если добавляете 49-ю карточку и далее — увеличьте верхнюю границу диапазона:

```python
COMMENTARY_IDS = [f"n{i}" for i in range(1, N+1)]  # для n<N>
```
